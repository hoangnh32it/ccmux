...
- **`internal/tui`** — Bubble Tea models and screens. Top-level app routes between Sessions, Projects, Conversations, Notes, Agents, Settings, Network, and Files screens. Lipgloss handles styling, Bubbles provides the list/table/textinput/viewport widgets.
...
- **`internal/filebrowser`** — the read-only operations layer behind the Files tab: whole-project tree walk (every regular file, same VCS/dependency/build pruning as `internal/notes`), root-containment-checked reads, fold/unfold tree logic as pure functions, binary-content sniffing, and chroma syntax highlighting with preview size caps. Deliberately separate from `internal/notes`, whose contract is "every `.md` file" — see [`docs/02_Architecture/07_File_Browser.md`](docs/02_Architecture/07_File_Browser.md).
...
- `docs/02_Architecture/07_File_Browser.md` — the Files tab: whole-project tree, chroma preview, cwd tracking, mouse, and why it is a separate screen from Notes
